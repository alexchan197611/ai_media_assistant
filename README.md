...
面向中文内容创作者的媒体生产工具。项目从最早的桌面版EXE能力迁移到WEB项目中，能力更强，也不会被操作系统、电脑平台所制约，方便所有人进行创作使用。也方便将来拓展到云端提供服务，对接在线大模型API实现更强的自动生成能力。

## 当前状态

Phase 0 工程基础已完成，比之前桌面版的1.0版本增强了很多，所以定义为2.0.0版本：React 编辑器、FastAPI API、SQLite 数据模型、Alembic 迁移、项目 CRUD、UUID 片段同步、自动保存和后台 Worker 边界均已建立；旧版非 GUI 的渲染、TTS、OmniVoice、BGM 选择等核心模块已接入 `media_core`，并通过后台任务 API 调用。
