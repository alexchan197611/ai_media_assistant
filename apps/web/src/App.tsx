...
export default function App() {
  ...
  const create = useMutation({ mutationFn: () => api.createProject('未命名视频'), onSuccess: value => openProject(value.id) })
  ...
  async function createNewProject() {
    const project = await api.createProject('未命名视频')
    queryClient.invalidateQueries({ queryKey: ['projects'] })
  ...
  if (projectId) {
    if (project.isError) return <div className="fatal">项目加载失败。<button onClick={() => openProject(null)}>返回首页</button></div>
    if (!draft) return <div className="loading">正在打开编辑器…</div>
  ...
  return <Home projects={projects.data ?? []} loading={projects.isLoading} onCreate={() => create.mutate()} onOpen={openProject} onDuplicate={id => duplicate.mutate(id)} onDelete={id => { if (confirm('确定删除这个项目吗？')) remove.mutate(id) }}/>
}
