...
class OmniVoiceGenerationCancelled(RuntimeError):
    pass
    ...
    def request(self, request_path: Path, result_path: Path, cancel_check: Callable[[], bool] | None = None) -> None:
        with self.lock:
            ...
            if cancel_check and cancel_check():
                raise OmniVoiceGenerationCancelled("用户已取消 OmniVoice 生成")
...
def generate_omnivoice_audio(texts: list[str], output_dir: Path, options: OmniVoiceOptions, cancel_check: Callable[[], bool] | None = None) -> list[Path]:
    if not texts:
        return []
    ...
    except OmniVoiceGenerationCancelled:
        raise
...
def apply_speed(audio, speed):
    ...
    if abs(speed - 1.0) < 0.01 or len(array) < 2:
        return array
    target_length = max(1, int(len(array) / speed))
...
def process_request(request_path, result_path):
    ...
    for index, text in enumerate(request["texts"], start=1):
        kwargs = {"text": text.strip(), "speed": 1.0, "num_step": num_step}
        if mode == "clone":
        ...
        audio = apply_speed(tts.generate(**kwargs), speed)
